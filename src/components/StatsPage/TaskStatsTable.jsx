...
const formatRow = (task) => {
  ...
  const taskHHMMSS = formatSecsHHMMSS(statusAge);
  const {from} = taskAttrs;
  return {sid, from, status, queue_name, taskHHMMSS, statusAge, channel_type, worker_name};
};
...
const metadata = {
  ...
  cols: [
    ...
    { id: 'taskHHMMSS', numeric: true, disablePadding: false, label: 'Task Time', sortFld: 'statusAge' },
